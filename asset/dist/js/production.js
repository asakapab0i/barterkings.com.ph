			...
			if (this.sliderElem) {
				...
				if (Array.isArray(this.options.ticks_labels) && this.options.ticks_labels.length > 0) {
					...
					for (i = 0; i < this.options.ticks_labels.length; i++) {
						...
						labelContent.innerHTML = this.options.ticks_labels[tickLabelsIndex];
   ...
   - Place it underneath this comment block and write its signature like so:
   	...
   	********************************/
			...
			_layout: function _layout() {
				...
				var labelValue = this._state.value[0];
				if (this._state.value[0] == this.options.max) {
					var labelValue = this.options.max + '+';
				};

				$(this.tickLabels[1]).children().html(labelValue);
